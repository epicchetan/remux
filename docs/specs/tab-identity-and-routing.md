...
- **Keep delivery identity stable across in-tab retargets.** For extension-routed RPCs, the runtime also derives an opaque `_remuxOrigin` used to target server-pushed streams back to one viewer. That origin is connection-scoped and keyed by `tabId`, not by the serialized full context: `resourceKey` is intentionally mutable as a viewer navigates or persists route state, while the viewer owning an active stream has not changed. The full serialized context continues to travel separately as `_remuxViewerKey` for current-resource provenance. A different tab gets a different origin, and a new downstream connection gets a new origin, preserving both cross-tab isolation and reconnect detection.
