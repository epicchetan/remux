...
2. **Git metadata watchers (instant, badge-level for index/HEAD ops).** For each known repo root, watch `.git/HEAD` and `.git/index`. Catches commits, stages/unstages, branch switches, rebases — none of which touch worktree files. Any event here also triggers an immediate run of layer 3 for that root.
