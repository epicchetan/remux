...
| C3 | P1 | `native-journal.ts:368`, `native-coordinator.ts:3826` | Concurrent commands with one command ID both execute; reproduced `UNIQUE constraint failed` on compaction. | Pass B | C3a, S2a1 root and S2a2 steer/manual Compact deployed; 292 server tests and live Codex steer/Compact passed; later delivery adoption remains S2 |
...
| H2 | P1 | `viewer_bundles.rs:170`, `:294` | Watcher registers only roots existing at startup. | Pass G | implemented/deployed 2026-09-06 (`96079fd`); missing-root and delete/recreate regressions; 11 watcher tests, 138 host library tests passed |
...
| H4 | P1 (plausible) | `viewer_bundles.rs:519` | Publication renames before insert; cleanup without the per-view lock can delete it. | Pass G | implemented/deployed 2026-09-06 (`96079fd`); shared per-view publication/cleanup lock and deterministic retention-overflow barrier regression; 138 host library tests passed |
...
| I8 | P1 | `useConversationActions.ts`, `identity.ts`, `composer/actions/turnAction.ts` | Creation commits just before restart but viewer loses acknowledgement; draft stays on New chat. First-message retries also regenerate IDs, and draft handoff can discard recovery identity before resource reads finish. | [New-chat recovery](agent-new-chat-recovery.md): persist exact create/first-message intent, read-only receipt lookup, bounded reconnect/reload recovery and explicit retry. | implemented and deployed (`73a2157`, `dc04358`); desktop/mobile live draft recovery preserves the existing conversation and text without sending; regression evidence in spec |
...
| Subsequent incident rows; acceptance recorded individually | 8 (I1–I8) |
| Reproduced implementation-stage findings | 1 (A13) |
| Total tracked rows | 93 |
