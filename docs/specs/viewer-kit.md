...
- **App owns the signal.** When `browserStore.activeTabId` changes to a viewer tab, or the app returns from OS background, the app emits a host event (working name `host/lifecycle/resume`) to that viewer over the existing `remux/event` channel.
- **Kit owns the ergonomics.** `useViewerResume(callback)` subscribes to that event. It replaces hand-wired `visibilitychange` for the tab-switch case and fixes the latent bug where a backgrounded-but-mounted codex tab never resyncs on return.
- **Viewer owns the reaction.** Codex's `resumeSync.ts` keeps its throttle / in-flight / connected-state orchestration but takes its resume trigger from `useViewerResume` instead of `document.visibilitychange`. Satellites get the hook available but no-op by default.
