...
Its non-blocking federation and automatic-continuation follow-up is
[agent-federation-nonblocking-orchestration.md](agent-federation-nonblocking-orchestration.md).
...
| [agent-federation-nonblocking-orchestration.md](agent-federation-nonblocking-orchestration.md) | Active Spec | Claude parents no longer block on federated children: Claude Code MCP auto-background (`CLAUDE_AUTO_BACKGROUND_TASKS` + 10 s threshold) ends the parent turn, the child result returns as a task notification and autonomous continuation; turn `origin` + transcript notice instead of a fake user bubble; lost-watcher fallback enqueue; plain one-click Send with a Delivery config row and footer reasons. |
...
| [viewer-direct-host-transport.md](viewer-direct-host-transport.md) | Implemented — live viewer verified; auth deployment pending | Standalone same-origin WebSocket host, shared route identity, disposable headless screenshot/CDP helper, and query-token cookie hand-off. |
