...
| [agent-thread-runtime-v1.md](agent-thread-runtime-v1.md) | Accepted implementation spec | Single-path replacement: versioned `thread.md`, bounded turn capsules and exact dialogue, turn/work-unit-local provider reasoning, actual context frames, and a fresh no-migration database. Supersedes the shadow/stateful/working-memory experiments. |
