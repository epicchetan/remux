...
| [agent-phase-1a0-implementation-report.md](agent-phase-1a0-implementation-report.md) | Active Spec | Phase 1A.0 source/deviation/test record. Automated implementation and hardening are complete; live desktop/physical-phone comparison and explicit owner acceptance remain open. |
| [agent-phase-1a1-durable-history-scope.md](agent-phase-1a1-durable-history-scope.md) | Proposed Spec | Proposed next checkpoint: SQLite journal/artifact foundation, deterministic replay, idempotent conversation creation/send, restart recovery, full-replay Pi hydration and budget guard, plus Agent-owned desktop/mobile conversation history. No epoch compilation, new coding effects, or later interaction features. |
