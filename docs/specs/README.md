...
| [agent-audit-2026-09-05-findings.md](agent-audit-2026-09-05-findings.md) | Audit record — 84 original findings, 4 incident follow-ups, 1 implementation finding; implementation tracked | Preserves all original finding IDs; planned coverage is separate from implementation status and evidence. Adds Claude context/compaction, timestamp/history-sync, Codex child-projection, and transcript error-geometry incidents, plus the reproduced Claude configuration return-switch finding, for 89 tracked rows. |
| [agent-audit-remediation-pass-1.md](agent-audit-remediation-pass-1.md) | Active Spec — staged implementation; incident fixes locally verified | S0–S7 delivery plan with immediate S0a Codex child-identity and S0b error-geometry slices and bounded Sol implementation assignments and primary review gates; explicit delivery evidence, focused ownership extraction, stable-ID reconciliation, one viewer sync controller, scoped migrations, and per-slice verification records. |
| [agent-subagent-lifecycle.md](agent-subagent-lifecycle.md) | Active Spec — current implementation priority | Durable agent/assignment identity, restart reconciliation, conversation/child Stop, evidence-based phantom repair and one stable bottom activity row. Broader audit remains paused. |
| [agent-command-delivery-s2a.md](agent-command-delivery-s2a.md) | Reviewed implementation contract — S2 cutover pending | Shared durable delivery attempts, exact native acceptance evidence, bounded staging and atomic admission; remediation pass 1 owns serial Sol assignments and verification status. |
