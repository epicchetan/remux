...
import { getConversationRuntimeState } from '../conversation/runtimeStore';
import {
...
> = {
  ...
  async setWidth(width) {
    ...
    if (!resourceSnapshot?.activeConversationId) {
      return;
    }
  ...
  toggleWorkDisclosure(input) {
    ...
    if (isCurrentlyOpen) {
      const runtime = getConversationRuntimeState();
      const closingRuntimeActiveTurn =
        (runtime.status === 'running' || runtime.status === 'interrupting') &&
...
export function reconcileTranscriptLayoutFromResources(
  ...
  if (!resourceSnapshot.activeConversationId || width === null) {
    return;
  }
  ...
  const runtime = getConversationRuntimeState();
  const autoOpenTurnId = runtime.status === 'running' || runtime.status === 'interrupting' ? runtime.activeTurnId : null;
