  ...
  return (
    ...
    >
      {document.blocks.map((block, index) => (
        <MarkdownBlockNode block={block} key={`${block.type}:${block.renderId}:${index}`} />
      ))}
    </div>
  );
}
...
const MarkdownBlockNode = memo(function MarkdownBlockNode({
  ...
  return (
    <div
      ...
      data-markdown-block-id={block.renderId}
      ...
      return (
        <blockquote className="codex-md-block codex-md-blockquote" style={style}>
          {block.children.map((child, index) => (
            <MarkdownBlockNode block={child} key={`${child.type}:${child.renderId}:${index}`} />
          ))}
        </blockquote>
      );
    case 'list':
              ...
              >
                ...
                <div className="codex-md-list-content">
                  {item.blocks.map((child, childIndex) => (
                    <MarkdownBlockNode block={child} key={`${child.type}:${child.renderId}:${childIndex}`} />
                  ))}
                </div>
              </div>
            </div>
...
function MarkdownLineFragment({
  ...
  if (source.kind === 'code') {
    ...
    return href ? (
      <a className="codex-md-line-code-link" href={href} onClick={(event) => handleAgentLinkClick(event, href)}>
        {code}
      </a>
    ) : (
      code
  ...
  if (source.href) {
    const href = source.href;
    return (
      <a className={inlineClassName(source)} href={href} onClick={(event) => handleAgentLinkClick(event, href)} style={style}>
        {fragment.text}
      </a>
    );
  }
...
function handleAgentLinkClick(event: MouseEvent<HTMLAnchorElement>, href: string) {
  event.preventDefault();
