...
import { useConversationRuntimeStore } from '../../../conversation/runtimeStore';
import {
...
export const WorkingDuration = memo(function WorkingDuration({
  ...
  const elapsedAnchorMs = useConversationRuntimeStore((state) =>
    state.activeTurnId === turnId && (state.status === 'running' || state.status === 'interrupting')
