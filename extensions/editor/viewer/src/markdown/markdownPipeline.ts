    ...
    visitMarkdownAst(tree, (node) => {
      ...
      node.data = {
        ...
        hProperties: {
          ...
          className: [
            'remux-viewer-markdown-alert',
            `remux-viewer-markdown-alert-${kind}`,
          ],
        },
      };
    });
...
// that logical newline, so remove the serialization-only copy.
...
export const remuxMarkdownSanitizeSchema = {
  ...
  attributes: {
    ...
    blockquote: [
      ...
      ['className', 'remux-viewer-markdown-alert', /^remux-viewer-markdown-alert-(?:note|tip|important|warning|caution)$/u],
    ],
    code: [
